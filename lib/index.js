...
(function() {
  ...
  EPub = class EPub {
    constructor(options,tempDirPath, output) {
      var self;
      ...
      if (!this.options.tempDir) {
        this.options.tempDir = tempDirPath;//path.resolve(__dirname, "./tmp/");
      }
      this.id = uuid();
      this.uuid = path.join(this.options.tempDir, this.id);
      ...
      this.options.content = _.map(this.options.content, function(content, index) {
        ...
        if (!content.filename) {
          titleSlug = uslug(removeDiacritics(content.title || content.type || "no title"));
          if(content.title === 'Referências Bibliográficas'){
            content.href = `${titleSlug}.xhtml`;
          }else{
            content.href = `${index}_${titleSlug}.xhtml`;
          }          
          content.filePath = path.join(self.uuid, `./OEBPS/${content.href}`);
        } else {
          content.href = content.filename.match(/\.xhtml$/) ? content.filename : `${content.filename}.xhtml`;
          if (content.filename.match(/\.xhtml$/)) {
            content.filePath = path.join(self.uuid, `./OEBPS/${content.filename}`);
          } else {
            content.filePath = path.join(self.uuid, `./OEBPS/${content.filename}.xhtml`);
          }
        }
        ...
        $("img").each(function(index, elem) {
          ...
          if (image = self.options.images.find(function(element) {
            ...
            id = uuid();
            mediaType = url ? mime.getType(url.replace(/\?.*/, "")) : url;
            extension = url ? mime.getExtension(mediaType): url;
            dir = content.dir;
      ...
      if (this.options.cover) {
        this.options._coverMediaType = mime.getType(this.options.cover.replace(/\?.*/, ""));
    ...
    generateTempFile() {
      ...
      fs.mkdirSync(path.join(this.uuid, "./OEBPS"));
      (base = this.options).css || (base.css = fs.readFileSync(path.resolve(__dirname, "../templates/template.css")));
      fs.writeFileSync(path.join(this.uuid, "./OEBPS/style.css"), this.options.css);
      if (self.options.fonts.length) {
        fs.mkdirSync(path.join(this.uuid, "./OEBPS/fonts"));
        this.options.fonts = _.map(this.options.fonts, function(font) {
          ...
          fsextra.copySync(font, path.join(self.uuid, "./OEBPS/fonts/" + filename));
          return filename;
        });
      }
      _.each(this.options.content, function(content) {
        var data;
        data = `${self.options.docHeader}\n  <head>\n  <meta charset="UTF-8" />\n  <title>${entities.encodeXML(content.title || '')}</title>\n  <link rel="stylesheet" type="text/css" href="style.css" />\n <style> ${content.style} </style> </head>\n<body>`;
        data += content.title && self.options.appendChapterTitles ? `<h1 id="${content.titleId}">${entities.encodeXML(content.title)}</h1>` : "";
      ...
      ncxTocPath = self.options.customNcxTocTemplatePath || path.resolve(__dirname, "../templates/tocSaitebooker.ncx.ejs");
      if (!fs.existsSync(ncxTocPath)) {
        generateDefer.reject(new Error('Custom file the NCX toc template not found.'));
        return generateDefer.promise;
      }
      htmlTocPath = self.options.customHtmlTocTemplatePath || path.resolve(__dirname, `../templates/epub${self.options.version}/tocSaitebooker.xhtml.ejs`);
      ...
      Q.all([Q.nfcall(ejs.renderFile, opfPath, self.options), Q.nfcall(ejs.renderFile, ncxTocPath, self.options), Q.nfcall(ejs.renderFile, htmlTocPath, self.options)]).spread(function(data1, data2, data3) {
        fs.writeFileSync(path.join(self.uuid, "./OEBPS/content.opf"), data1);
        fs.writeFileSync(path.join(self.uuid, "./OEBPS/toc.ncx"), data2);
        fs.writeFileSync(path.join(self.uuid, "./OEBPS/toc.xhtml"), data3);
    ...
    makeCover() {
      ...
      if (this.options.cover) {
        destPath = path.join(this.uuid, "./OEBPS/cover." + this.options._coverExtension);
        writeStream = null;
    ...
    downloadImage(options) { //{id, url, mediaType}
      ...
      filename = path.join(self.uuid, "./OEBPS/images/" + options.id + "." + options.extension);
      if (options.url.indexOf("file://") === 0) {
        ...
        if (options.url.indexOf("http") === 0) {
          ...
          requestAction = fs.createReadStream(path.join(options.dir, options.url));
          requestAction.pipe(fs.createWriteStream(filename));
        }
    ...
    downloadAllImage() {
      ...
      if (!self.options.images.length) {
        ...
        fs.mkdirSync(path.join(this.uuid, "./OEBPS/images"));
        deferArray = [];
